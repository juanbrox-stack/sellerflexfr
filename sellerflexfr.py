...
stock_file = st.sidebar.file_uploader("1. Fichero de Stock FR (CSV)", type=["csv", "xlsx"])
pedidos_recoger_file = st.sidebar.file_uploader("2. Fichero PedidosRecoger", type=["csv", "xlsx"])
listar_recogida_file = st.sidebar.file_uploader("3. Fichero ListarRecogida", type=["csv", "xlsx"])
plantilla_file = st.sidebar.file_uploader("4. Fichero Plantilla SELLER_FLEX_FR", type=["csv", "xlsx"])

def load_data(file):
    """Carga los ficheros manejando errores de codificación (UnicodeDecodeError) y delimitadores europeos (punto y coma)."""
    if file is not None:
        if file.name.endswith('.csv'):
            try:
                # Intento estándar UTF-8 con autodetección de separador
                return pd.read_csv(file, sep=None, engine='python')
            except Exception:
                # Fallback para codificaciones de Windows/Excel en Europa Occidental (ISO-8859-1 / Latin-1)
                file.seek(0)
...
if stock_file and pedidos_recoger_file and listar_recogida_file and plantilla_file:
    ...
    try:
        ...
        # La columna A es la primera columna del fichero de stock
        ...
        col_listar_a = df_listar_recogida.columns[0] # 'ID del pedido'
        
        def parse_listar_recogida(text):
            text = str(text).strip()
            # Extrae el número de pedido (ej: 406-4244722-3978723) que va antes del primer espacio
        ...
        def mapear_estado(id_envio):
            if id_envio in envios_cancelar:
                return 'CANCELAR'
            return 'OK' # Por defecto u OK si hay stock disponible
        ...
        df_zona_mapping = df_pedidos_recoger[['Identificador de pedido', 'Zona', 'FNSKU']].drop_duplicates()
        ...
        # 1. Fichero Fichero Resultante Cecopartners
        ...
        if not df_ok.empty:
            ...
            # Generación de códigos consecutivos simulando la secuencia D26600114...
        ...
        st.success("✨ ¡Ficheros cruzados y procesados de forma correcta!")
        ...
        st.error(f"Error estructural al procesar las columnas: {e}")
        st.warning("Verifica que las cabeceras internas de tus ficheros coinciden con la configuración estándar ('SKU', 'Zona', 'Identificador de pedido').")
else:
