...
def enviar_correo_almacen_francia(excel_data, filename):
    try:
        ...
        msg['Subject'] = f"Fichero D-PEDIDOS FLEX FR"
    ...
    if df_stock is None or df_stock.empty:
        ...
        try:
            ...
            def parse_listar_recogida(text):
                ...
                # Separación de longitud fija: los últimos 9 caracteres corresponden a la U (Shipment ID)
            ...
            # Mapeos auxiliares guardados en session_state para evitar pérdidas en re-runs
            mapa_p = {}
            mapa_u = {}
                ...
                if cecopartners_downloaded_file is not None:
                    ...
                    if df_ceco_in is not None and not df_ceco_in.empty:
                        try:
                            ...
                            # Priorizar 'número de línea de pedido de cliente' que hospeda los 19 dígitos de Amazon (4XX-)
                            col_ceco_pedido_real = dict_cols_ceco.get('número de línea de pedido de cliente', col_ceco_pedido)
                            pedidos_ceco_limpios = df_ceco_in[col_ceco_pedido_real].astype(str).str.strip()
                            
                            mapa_zona_session = st.session_state.get('mapa_pedido_a_zona', {})
                            mapa_envio_session = st.session_state.get('mapa_pedido_a_envio', {})
                            
                            # Generar las tres nuevas hileras inyectadas al principio
                            ...
                            # Borrar la columna A (primera columna del archivo Cecopartners)
                            df_ceco_restante = df_ceco_in.iloc[:, 1:].copy()
                            
                            # Reensamblar el archivo: P, U y Agencia al principio, seguido del resto de Cecopartners intacto
                            ...
                            nombre_archivo_dpedidos = f"D-PEDIDOS_FLEX_FR_20260521.xlsx"
